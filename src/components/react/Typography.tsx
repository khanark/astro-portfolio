...
const PrimaryHeading = ({
    ...
    return (
        <h2
            className={`${style.textPrimary} font-semibold text-3xl mb-8 ${classes}`}
        >
            {children}
        </h2>
    );
...
const Paragraph = ({
    ...
    return (
        <p className={`${style.textSecondary} mb-8 ${classes}`}>{children}</p>
    );
};
...
const Typography = ({
    ...
    switch (variant) {
        ...
        case "h2":
            return (
                <PrimaryHeading classes={className}>{children}</PrimaryHeading>
            );
        case "h3":
